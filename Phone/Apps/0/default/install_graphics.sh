...
# This script is meant to copy all Graphic files from the source to MIB and backup the original ones
# author: Superkolos
########################

#info
TOPIC=Graphics
DESCRIPTION="This script will copy custom Graphic files from Graphics folder on your SD to the unit."
...
cp -R /$MIBFOLDER/ $BACKUPFOLDER

echo Copying modified files from SD Graphics folder to MIB.
cp -R $SOURCEFOLDER/* /eso/hmi/lsd/images/
...
echo Sometimes it takes a wile until new graphics appear in the menus.
echo You can shut down the car and wait approx 20min to affect changes.
